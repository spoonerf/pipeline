...
pr_in_files <-
  list.files(here::here("environmental/presence"),
             full.names = TRUE,
             pattern = "*.csv")

bkg_in_files <-
  paste0(here::here("environmental/background"),
                          "/",
                          basename(pr_in_files))


pr_in_files <- pr_in_files[basename(pr_in_files) %in% list.files(here::here("environmental/background"))]


fitBC(pres_file_in = pr_in_files[6],
           backg_file_in = bkg_in_files[6],
...
###Fit Domain - very slow? 
```{r}

if(!dir.exists(here::here("predictions/domain"))){
  dir.create(here::here("predictions/domain"))
  }

fitDomain(pres_file_in = pr_in_files[3],
           backg_file_in = bkg_in_files[3],
           predictor_names = c(1,3,4,12),
           predictors = env_crop,
           out_dir = here::here("predictions/domain/"),
           overwrite = TRUE,
           threads = 4, 
           eval = TRUE)
