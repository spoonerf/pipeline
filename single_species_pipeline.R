...
#We are only interested in species that occur in the study region - covered in the extents below
...
min_occ <- 40 # minimum number of occurrence points needed for the species to be included in the models
...
env_layers <- loadBioclim(path = ("CHELSA/"), extension = ".tif")  #add the path linking to your environment variables.
...
sp_name <- "Lophorina_superba"  #add species name with underscore between genus and species
...
spxy_out <- gbifData(sp_name = sp_name, 
...
background_sampler(sp_name = sp_name, 
                   in_dir = here::here("points/rarefied"),
...
preds <- list.files(here::here("predictions"), full.names = TRUE, recursive = TRUE, pattern =  paste0("*", sp_name))
...
maj_pa<- ensemble_model(sp_name = sp_name, 
               eval_df = eval_df, 
               preds = preds,
               method = "majority_pa", 
               out_dir = here::here("predictions/ensemble"))

weighted <- ensemble_model(sp_name = sp_name, 
                           eval_df = eval_df, 
                           preds = preds,
                           method = "weighted", 
                           out_dir = here::here("predictions/ensemble"))
